...
class MolDock:
    ...
    def _dock_table_with_unidock_single(self, selected_table, selected_method, assay_registry, clean_ligand_files, receptor_info):
        import sqlite3
        import tempfile
        import os
        import subprocess
    ...
    def _execute_unidock(self, ligand_pdbqt_filepath, assay_registry, method_params, receptor_pdbqt_file, receptor_conditions, unidock_cmd):

        import subprocess

        ## Append ligand path to unidock command
        unidock_cmd += f"--ligand {ligand_pdbqt_filepath} "
    ...
    def compute_fingerprints(self, minimize=True, clean_files=True, write_prolif=False, write_mmgbsa=False, selection_threshold=25, max_workers=None):
        ...
        worker count), prompts whether to run the computation — ligand preparation
        followed by the parallel per-pose dispatch — in the foreground (blocks until
        done) or background (detached process, launched before ligand preparation
        starts; progress/errors go to a log file under the assay's logs/ folder, and
        this call returns immediately). Defaults to foreground.
        """
        ...
        # worker-count prompt) always happens here in the foreground. The user is
        # then asked whether the actual computation — ligand preparation and the
        # parallel dispatch that follows it — should run in the foreground or
        # background, before any of that work starts.
        prep = self._prepare_fingerprints_execution(
            df, results_db, assay_info, renumbering_dict, max_workers,
            prolif, mmbgsa, prolif_params_dict, condition_selection,
            write_prolif, write_mmgbsa,
        )
        if prep is None:
            return None

        pending, mode_label, resolved_max_workers = prep

        exec_kwargs = dict(
            pending=pending, mode_label=mode_label, prolif=prolif, mmgbsa=mmbgsa,
            results_db=results_db, condition_selection=condition_selection,
            clean_files=clean_files, write_prolif=write_prolif, write_mmgbsa=write_mmgbsa,
            assay_info=assay_info, max_workers=resolved_max_workers, computation_mode=computation_mode,
            is_stub_receptor=is_stub_receptor, fps_tleap_config=fps_tleap_config, minimize=minimize,
            renumbering_dict=renumbering_dict, prolif_params_dict=prolif_params_dict,
        ...
        if run_mode in ('fg', 'foreground'):
            ...
            self._run_fingerprints_computation(**exec_kwargs)
    ...
    def _prepare_fingerprints_execution(self, df, results_db, assay_info, renumbering_dict,
                                        max_workers, prolif, mmgbsa, prolif_params_dict,
                                        condition_selection, write_prolif, write_mmgbsa):
        """
        Interactive setup phase for compute_fingerprints(): resolves resume/overwrite
        choices and the worker-count prompt — the only steps that call input(). This
        phase always runs synchronously in the foreground, before the user is asked
        whether the actual computation (ligand preparation + parallel dispatch, in
        _run_fingerprints_computation()) should run in the foreground or background.
        ...
        Returns (pending, mode_label, max_workers) ready to hand to
        _run_fingerprints_computation(), or None if there is nothing to compute (in
        ...
        return pending, mode_label, max_workers

    def _run_fingerprints_computation(self, pending, mode_label, prolif, mmgbsa, results_db,
                                       condition_selection, clean_files, write_prolif, write_mmgbsa,
                                       assay_info, max_workers, computation_mode,
                                       is_stub_receptor, fps_tleap_config, minimize,
                                       renumbering_dict, prolif_params_dict, receptor_file):
        """
        Non-interactive execution phase for compute_fingerprints(): prepares
        per-ligand parameter files, builds the per-pose task list, dispatches it to
        a ProcessPoolExecutor, collects and stores results, then runs the requested
        exports and cleanup. Each docked pose's shared pipeline (restore -> build
        complex -> minimize) runs once per pose in its own worker process, isolated
        in its own subdirectory under temp_restored_poses/, so concurrent poses
        never collide on the fixed-name scratch files written by
        ...
        restores/builds/minimizes a pose twice. GPU minimization is serialized
        across workers via a shared lock so only one worker uses pmemd.cuda at a
        time. All SQLite writes to results_db happen back in this process as
    ...
    def _launch_fingerprints_background(self, **exec_kwargs):
        """
        Launch _run_fingerprints_computation() as a fully detached background process
        (survives terminal/SSH disconnect), mirroring the nohup-style launch used by
        _run_docking_background(). Unlike that method, exec_kwargs is pickled to disk
        rather than embedded as literal Python source in the generated script: it
        carries `pending`, a list of (pandas row, do_prolif, do_mmgbsa) tuples — one
        per docked pose — whose pandas/numpy scalar values aren't safely
        representable via repr().
        """
        import pickle
        import subprocess
        import sys
        ...
        python_script = textwrap.dedent(f"""
            ...
            try:
                mdock._run_fingerprints_computation(**payload['exec_kwargs'])
    ...
    def _minimize_complex(self, prmtop_file, inpcrd_file, output_dir, ligname, pose_id, output_pdb_file, gpu_lock=None):
        ...
        # fingerprint computation path (_run_fingerprints_computation) and a
        ...
        if gpu_available:
            # Run minimization with GPU acceleration
            print("GPU detected. Using pmemd.cuda for minimization...")
    ...
    def _compute_prolif_fingerprints(self, prmtop_file, min_rst_cpptraj_file, pose_id, results_db, prolif_params_dict):
        ...
        prot = u.select_atoms(prolif_params_dict['selection_dict']['receptor'])
    ...
    def _check_gpu_available(self):
        """Check if CUDA GPU is available by testing nvidia-smi"""
    ...
    def create_prolif_conditions(self):
        """
        Will create a dictionary containing ProLIF parameters to be used in the corresponding fingerprints analyses
        """

        # Create a dictionary to store ProLIF parameters
        prolif_conditions = {}

        ## Populate the conditions dictionary

        # Define interactions to consider
        interactions_list = self._get_prolif_interaction_list()
        
        prolif_conditions['interactions_list'] = interactions_list

        # Create the receptor MDAnalysis selection command
        selection_dict = self._create_selection_commands()
        prolif_conditions['selection_dict'] = selection_dict

        # Define parameters for each interaction type
        interaction_parameters = {}

        print(f"\n⚙️  CONFIGURE INTERACTION-SPECIFIC PARAMETERS")
        print("=" * 70)
        ...
        for interaction in interactions_list:
            ...
            if interaction == 'Anionic':
                
                cationic_dict =self._define_cationic_interaction(interaction)

                interaction_parameters['Cationic'] = cationic_dict

            elif interaction == 'CationPi':

                cationpi_dict =self._define_cationpi_interaction(interaction)

                interaction_parameters['CationPi'] = cationpi_dict
                
            elif interaction == 'EdgeToFace':

                edgetoface_dict =self._define_edge_to_face_interaction(interaction)

                interaction_parameters['EdgeToFace'] = edgetoface_dict
                
            elif interaction == 'FaceToFace':
                
                faceto_face_dict =self._define_face_to_face_interaction(interaction)
                
                interaction_parameters['FaceToFace'] = faceto_face_dict

            elif interaction == 'HBAcceptor':
                
                hbacceptor_dict =self._define_hbacceptor_interaction(interaction)
                
                interaction_parameters['HBAcceptor'] = hbacceptor_dict
                ...
                interaction_parameters['Hydrophobic'] = hydrophobic_dict
                ...
                metaldonor_dict =self._define_metaldonor_interaction(interaction)
                
                interaction_parameters['MetalDonor'] = metaldonor_dict
                ...
                print("PiStacking parameters are derived from FaceToFace and EdgeToFace interactions.")
                
                # Check if FaceToFace and EdgeToFace exists in list, otherwise inform and stop
                if "FaceToFace" not in interactions_list and "EdgeToFace" not in interactions_list:
                    print("❌ FaceToFace and EdgeToFace interactions must be defined to set parameters for PiStacking.")
                    sys.exit(1)
                
                pass
                
            elif interaction == 'VdWContact':
                
                vdwcontact_dict =self._define_vdwcontact_interaction(interaction)
                
                interaction_parameters['VdWContact'] = vdwcontact_dict
                
            elif interaction == 'XBAcceptor':
                
                xbacceptor_dict =self._define_xbacceptor_interaction(interaction)
                
                interaction_parameters['XBAcceptor'] = xbacceptor_dict
    ...
    def _get_prolif_parameter_string(self, param_name: str, default: str, description: str = None) -> str:
        ...
        Returns:
            str: The parameter value entered by user or default
        """
        try:
            print(f"\n📋 {param_name}:")
            if description:
            ...
            prompt = f"Enter value for {param_name} (default: {default}): "
            value = input(prompt).strip()
    ...
    def _get_prolif_parameter_float(self, param_name: str, default: float, 
                                    min_val: float = 0.0, max_val: float = 100.0,
                                    description: str = None) -> float:
        """
        Get float parameter for ProLIF conditions with validation.
        ...
        Returns:
            float: The parameter value entered by user or default
        """
        while True:
            try:
                print(f"\n📊 {param_name}:")
    ...
    def _get_prolif_parameter_bool(self, param_name: str, default: bool = True,
                                description: str = None) -> bool:
        """
        Get boolean parameter for ProLIF conditions.
        ...
        Returns:
            bool: The parameter value entered by user or default
        """
        while True:
            try:
                print(f"\n❓ {param_name}:")
                ...
                prompt = f"Enable {param_name}? (yes/no, default: {default_str}): "
    ...
    def _get_prolif_interaction_list(self, default_interactions: List[str] = None) -> List[str]:
        """
        Get list of ProLIF interactions to compute.
        
        Args:
            default_interactions (List[str]): Default list of interactions
            
        Returns:
            List[str]: Selected interactions list
        """
        try:
            if default_interactions is None:
            ...
            print("=" * 70)
            print("Choose which interactions to compute:")
            print("\nAvailable interactions:")
            for i, interaction in enumerate(default_interactions, 1):
                print(f"   {i:2d}. {interaction}")
            
            print("\n" + "=" * 70)
            print("Options:")
            print("   1. Use ALL interactions (default)")
            print("   2. Select specific interactions")
            print("=" * 70)
            ...
            print("\n❌ Interaction selection cancelled.")
            raise

    def _define_cationic_interaction(self, interaction):

        cationic_dict = {}

        cationic_dict['cation'] = self._get_prolif_parameter_string(f"{interaction} cations list", default='[+{1-},$([NX3&!$([NX3]-O)]-[C]=[NX3+])]', description="SMARTS pattern for cations")

        cationic_dict['anion'] = self._get_prolif_parameter_string(f"{interaction} anions list", default='[-{1-},$(O=[C,S,P]-[O-])]', description="SMARTS pattern for anions")

        cationic_dict['distance'] = self._get_prolif_parameter_float(f"{interaction} distance", default=4.5, min_val=0.0, description="Distance cutoff Cationic for interaction")

        return cationic_dict

    def _define_cationpi_interaction(self, interaction):

        cationpi_dict = {}

        # Define cations
        cationpi_dict['cation'] = self._get_prolif_parameter_string(f"{interaction} cations list", default='[+{1-},$([NX3&!$([NX3]-O)]-[C]=[NX3+])]', description="SMARTS pattern for cations")
        ...
        cationpi_dict['pi_ring'] = (cation_pi_ring_1, cation_pi_ring_2)

        # Define distance cutoff

        cationpi_dict['distance'] = self._get_prolif_parameter_float(f"{interaction} distance", default=4.5, description="Distance cutoff CationPi for interaction")

        # Define angle cutoff

        angle_1 = self._get_prolif_parameter_float(f"{interaction} angle1", default=0.0, description="Minimum angle cutoff CationPi for interaction")

        angle_2 = self._get_prolif_parameter_float(f"{interaction} angle2", default=30.0, description="Maximum angle cutoff CationPi for interaction")

        cationpi_dict['angle'] =  (angle_1, angle_2)
    ...
    def _define_edge_to_face_interaction(self, interaction):
        ...
        edge_to_face_dict['distance'] = self._get_prolif_parameter_float(f"{interaction} distance", default=6.5, description="Cutoff distance between each rings centroid")
        
        # Define the plane_angle parameter
        angle_1 = self._get_prolif_parameter_float(f"{interaction} plane_angle angle1", default=50.0, description="Min value for the angle between the ring planes")

        angle_2 = self._get_prolif_parameter_float(f"{interaction} plane_angle angle2", default=90.0, description="Max value for the angle between the ring planes")

        edge_to_face_dict['plane_angle'] =  (angle_1, angle_2)
        
        # Define the normal_to_centroid_angle parameter
        angle_1 = self._get_prolif_parameter_float(f"{interaction} normal_to_centroid_angle angle1", default=0.0, description="Min value for the angle between the ring planes")

        angle_2 = self._get_prolif_parameter_float(f"{interaction} normal_to_centroid_angle angle2", default=30.0, description="Max value for the angle between the ring planes")

        edge_to_face_dict['normal_to_centroid_angle'] =  (angle_1, angle_2)
        
        # Define the interacting pi-ring SMARTS definitions
        ring_1 = self._get_prolif_parameter_string(f"{interaction} ring_1", default="[a;r6]1:[a;r6]:[a;r6]:[a;r6]:[a;r6]:[a;r6]:1", description="List of SMARTS for aromatic rings") 
        
        ring_2 = self._get_prolif_parameter_string(f"{interaction} ring_2", default="[a;r5]1:[a;r5]:[a;r5]:[a;r5]:[a;r5]:1", description="List of SMARTS for aromatic rings") 

        edge_to_face_dict['pi_ring'] =  (ring_1, ring_2)
        
        # Define the intersect point between ring planes
        edge_to_face_dict['intersect_radius'] = self._get_prolif_parameter_float(f"{interaction} intersect_point", default=1.5, description="Intersect point between ring planes")
        
        return edge_to_face_dict
    
    def _define_face_to_face_interaction(self, interaction):
        
        face_to_face_dict = {}
        
        # Define the distance cutoff for the interaction
        face_to_face_dict['distance'] = self._get_prolif_parameter_float(f"{interaction} distance", default=5.5, description="distance between each rings centroid")
        
        # Define the plane_angle parameter
        angle_1 = self._get_prolif_parameter_float(f"{interaction} plane_angle angle1", default=0.0, description="Min value for the angle between the ring planes")

        angle_2 = self._get_prolif_parameter_float(f"{interaction} plane_angle angle2", default=35.0, description="Max value for the angle between the ring planes")

        face_to_face_dict['plane_angle'] =  (angle_1, angle_2)
        
        # Define the normal_to_centroid_angle parameter
        angle_1 = self._get_prolif_parameter_float(f"{interaction} normal_to_centroid_angle angle1", default=0.0, description="Min value for the angle between the ring planes")

        angle_2 = self._get_prolif_parameter_float(f"{interaction} normal_to_centroid_angle angle2", default=33.0, description="Max value for the angle between the ring planes")

        face_to_face_dict['normal_to_centroid_angle'] =  (angle_1, angle_2)        
        ...
        face_to_face_dict['pi_ring'] =  (ring_1, ring_2)
        
        return face_to_face_dict
    
    def _define_hbacceptor_interaction(self, interaction):
        ...
        hbacceptor_dict['acceptor'] =  acceptor
        ...
        hbacceptor_dict['donor'] =  donor
        
        # Define the distance cutoff for the interaction
        hbacceptor_dict['distance'] = self._get_prolif_parameter_float(f"{interaction} distance", default=3.5, description="Distance cutoff for hydrogen bond interaction")
        ...
        hbacceptor_dict['DHA_angle'] =  (angle_1, angle_2) 
    ...
    def _define_hydrophobic_interaction(self, interaction):
        ...
        hydrophobic_dict['hydrophobic'] = self._get_prolif_parameter_string(f"{interaction} hydrophobic list", default='[c,s,Br,I,S&H0&v2,$([D3,D4;#6])&!$([#6]~[#7,#8,#9])&!$([#6X4H0]);+0]', description="SMARTS query for hydrophobic atoms")

        hydrophobic_dict['distance'] = self._get_prolif_parameter_float(f"{interaction} distance", default=4.5, description="Distance threshold for the hydrophobic interaction")

        return hydrophobic_dict

    def _define_metaldonor_interaction(self, interaction):
        ...
        metal_donor_dict['ligand'] =  ligand
    ...
    def _define_vdwcontact_interaction(self, interaction):
        ...
        # Setup a 'Tolerance' parameter

        tolerance = self._get_prolif_parameter_float(f"{interaction} tolerance", default=0.0, description="Tolerance added to the sum of vdW radii of atoms before comparing to the interatomic distance")

        vdwcontact_dict['tolerance'] = tolerance

        return vdwcontact_dict
    
    def _define_xbacceptor_interaction(self, interaction):
        
        xbacceptor_interaction_dict = {}
        ...
        xbacceptor_interaction_dict['acceptor'] =  acceptor
        ...
        xbacceptor_interaction_dict['donor'] =  donor
        ...
        xbacceptor_interaction_dict["distance"] = distance
        ...
        xbacceptor_interaction_dict['AXD_angle'] =  (angle_1, angle_2)
    ...
    def _register_prolif_conditions(self, prolif_conditions):
        ...
        """
        Will store the prolif_conditions dictionary as a registry in prolif_params.db located in docking/params folder.
        Prompts the user for a description and stores it along with the conditions as JSON string.
        """
        ...
        try:
            ...
            # Prompt user for conditions description
            print(f"\n📝 PROLIF CONDITIONS DESCRIPTION")
            print("=" * 70)
            # Loop until a non-empty description is provided
            while True:
                description = self._input_parameter_choice(
                    param_name="Conditions description",
                    default=None
                )
            ...
            # Serialize the conditions dictionary to JSON string
            conditions_string = json.dumps(prolif_conditions, indent=2)
            # Insert the new record
            cursor.execute("""
                INSERT INTO ProLIF_Conditions (description, conditions)
                VALUES (?, ?)
            """, (description, conditions_string))
            conn.commit()
            conn.close()
            print(f"\n✅ ProLIF conditions registered successfully in {prolif_params_db}")
            print(f"   📌 Description: {description}")
        
        except Exception as e:
            print(f"\n❌ Failed to register ProLIF conditions: {e}")

    def _get_prolif_params(self):

        """
        Will connect to the self.__docking_params_db, will list the available registers in the ProLIF_Conditions table showing the id and description table. Selection will retrieve the value stored in the conditions columns and return it as a dictionary
        """
        ...
        try:
            ...
            cursor.execute("""
                SELECT id, description FROM ProLIF_Conditions
                ORDER BY id ASC
            """)
            ...
            while True:
                try:
                    selection = input("\nSelect conditions (enter number or 'c' to cancel): ").strip()
                    ...
                    if 0 <= selection_idx < len(records):
                        selected_id = records[selection_idx][0]
                        break
                    else:
            ...
            # Retrieve the selected conditions
            cursor.execute("""
                SELECT description, conditions FROM ProLIF_Conditions
                WHERE id = ?
            """, (selected_id,))
    ...
    def _create_selection_commands(self):
        ...
        """
        Will query the user to enter a selection command por MDAnalysis receptor selection and ligand selection. The default for receptor is 'protein' and for ligand 'resname UNL'. 
        """
        ...
        selection_dict = {}
        # Receptor selection
        receptor_selection = self._input_parameter_choice(
            param_name="Receptor selection command",
            default="protein"
        )
        selection_dict['receptor'] = receptor_selection
        # Ligand selection
        ligand_selection = self._input_parameter_choice(
            param_name="Ligand selection command",
            default="resname UNL"
        )
        selection_dict['ligand'] = ligand_selection
    ...
    def _create_full_size_interactions_df(self, prolif_params_dict, renumbering_dict):
    
        import pandas as pd
    
        interactions_list = prolif_params_dict.get('interactions_list', [])
        
        # For each residue (renumbering_dict value), create columns for each interaction type
        columns = []
        for residue in renumbering_dict.values():
            for interaction in interactions_list:
                columns.append(f"{residue}_{interaction}")

        # Create an empty DataFrame with these columns
        full_size_df = pd.DataFrame(columns=columns)
            
        return full_size_df

    def _store_full_size_interactions_df_in_db(self, full_size_df, results_db):
        
        import sqlite3
        import json
        try:
            # Convert DataFrame to JSON string
            full_size_json = full_size_df.to_json(orient='split')
            conn = sqlite3.connect(results_db)
            cursor = conn.cursor()
            # Create a table if it doesn't exist
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS full_size_interactions_json (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    data TEXT NOT NULL
                )
            ''')
            ...
            cursor.execute('''
                INSERT INTO full_size_interactions_json (data) VALUES (?)
            ''', (full_size_json,))
            conn.commit()
            conn.close()
        except Exception as e:
    ...
    def _write_prolif_fps(self, results_db, assay_info):
        ...
        try:
            ...
            # Retrieve full size interactions DataFrame
            cursor.execute('''
                SELECT data FROM full_size_interactions_json LIMIT 1
            ''')
            result = cursor.fetchone()
            if result:
            ...
            # Merge full_size_df and complete_fps_df by column name (union of columns)
            merged_df = pd.concat([full_size_df, complete_fps_df], axis=0, ignore_index=True, sort=False)
            ...
            output_csv_serie = f"{assay_path}/results/prolif_fingerprints_serie.csv"
            merged_df.to_csv(output_csv, index=False)
            complete_fps_df.to_csv(output_csv_serie, index=False)

            print(f"\n✅ ProLIF fingerprints full size dataframe reconstructed and saved to '{output_csv}'")

            print(f"\n✅ ProLIF fingerprints serie dataframe reconstructed and saved to '{output_csv_serie}'")
...
# --- Module-level worker functions for MolDock._run_fingerprints_computation() ---
#
