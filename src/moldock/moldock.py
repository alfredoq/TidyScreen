...
quit
    ...
    def compute_fingerprints(self):
        ...
        for idx, row in df.iterrows():
            ...
            # Generate a dictionary matching the crystallographic numbering with that generated by tleap
            ## Under development
            crystal_tleap_correspondence_dictionary = self._create_crystal_tleap_correspondence_dictionary(assay_info)
            ...
            if prolif == True:
                ...
                # Flatten the MultiIndex columns by mapping the original second level to the origina third level
...
quit
    ...
    def _compute_prolif_fingerprints(self, prmtop_file, min_rst_cpptraj_file, pose_id, results_db, prolif_params_dict):
        ...
        # Flatten the structure of 'fps_df'
    ...
    def _create_crystal_tleap_correspondence_dictionary(self, assay_info):
        ...
        try:
            ...
            # Get the template name stored in the assay info in order to retrieve the raw receptor pdb file
            pdb_template_name = assay_info.get('receptor_info', None).get('template_name', None)
            receptors_db_path = os.path.join(self.path, 'docking', 'receptors', 'pdbs.db')
            
            if not os.path.exists(receptors_db_path):
                print(f"❌ Receptors database not found: {receptors_db_path}")
                return None
            
            conn = sqlite3.connect(receptors_db_path)
            cursor = conn.cursor()
            
            # Retrieve receptor information
            cursor.execute("""
                SELECT processed_pdb_path
                FROM pdb_templates
                WHERE pdb_template_name = ?
                LIMIT 1
            """, (pdb_template_name,))
            ...
            if result:
                processes_pdb_path = result[0]
            else:
                print(f"❌ No receptor information found for template: {pdb_template_name}")
                return None
            
        except Exception as e:
            print("Error conecting to receptors database:", e)

        pdbqt_file = assay_info.get('receptor_info', None).get('pdbqt_file', None)
        receptor_charge_model = assay_info.get('receptor_info', None).get('configs', None).get('receptor_charge_model',None)
        #crystal_pdb_file = pdbqt_file.replace(f'_{receptor_charge_model}.pdbqt', '.pdb')
        crystal_pdb_file = processes_pdb_path
        
        print(crystal_pdb_file)
        
        # Construct output dir
        assay_folder = assay_info.get('assay_folder_path', None)
        output_dir = os.path.join(assay_folder, 'results', 'temp_restored_poses')

        # Generate the tleap version of the pdb file 
        tleap_pdb = self._generate_tleap_receptor_pdb_file(crystal_pdb_file, output_dir)

        # Create the corresponding residue key lists
        crystal_keys_list = self._create_residues_key_list(crystal_pdb_file)
        tleap_keys_list = self._create_residues_key_list(tleap_pdb)

        print(crystal_keys_list)
        print(tleap_keys_list)
        

        # Check if both lists are of equal length. If not, inform and exit
        if len(crystal_keys_list) != len(tleap_keys_list):
            print("❌ Crystal and tleap receptor pdb files have different number of residues.")
            sys.exit(1)

        # Create correspondence dictionary
        correspondence_dict = {}
        for c_key, t_key in zip(crystal_keys_list, tleap_keys_list):
            correspondence_dict[t_key] = c_key

        # Store correspondence dictionary in assay_info database
        # Get the assay database path from assay_info
        assay_db_path = os.path.join(assay_info['assay_folder_path'], 'results', f"assay_{assay_info['assay_id']}.db")

        if not os.path.exists(assay_db_path):
            print(f"❌ Assay database not found: {assay_db_path}")
            return correspondence_dict

        # Save the correspondence dictionary as JSON to the assay_db
        try:
            import sqlite3
            import json
            
            conn = sqlite3.connect(assay_db_path)
            cursor = conn.cursor()
            
            # Create a table to store the correspondence dictionary as JSON if it doesn't exist
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS Receptor_Correspondence (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    correspondence_data TEXT NOT NULL,
                    description TEXT,
                    created_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                )
            """)
            ...
            correspondence_json = json.dumps(correspondence_dict, indent=2)
            
            # Insert or replace the correspondence data as a single JSON object
            cursor.execute("""
                DELETE FROM Receptor_Correspondence
            """)
            
            description = "Keys corresponds to tleap mapping, values to crystallographic ones"
            
            cursor.execute("""
                INSERT INTO Receptor_Correspondence (correspondence_data, description)
                VALUES (?, ?)
            """, (correspondence_json, description,))
            
            conn.commit()
            conn.close()
            
            print(f"✅ Receptor correspondence dictionary stored in assay database as JSON")
            print(f"   📊 Stored {len(correspondence_dict)} residue mappings")
            
        except sqlite3.Error as e:
            print(f"❌ Error storing correspondence dictionary: {e}")

        
        return correspondence_dict

    def _generate_tleap_receptor_pdb_file(self, pdb_file, output_dir):
        ...
        tleap_in_file = f"{output_dir}/tleap.in"
        tleap_out_pdb = f"{output_dir}/tleap.pdb"
        receptor_prmtop = f"{output_dir}/receptor.prmtop"
        receptor_inpcrd = f"{output_dir}/receptor.inpcrd"

        with open(tleap_in_file, 'w') as f:
            f.write(f"""source leaprc.protein.ff14SB
        source leaprc.water.tip3p
        HOH = WAT
        rec = loadpdb "{pdb_file}"
        saveamberparm rec "{receptor_prmtop}" "{receptor_inpcrd}"
        quit
        """)

        # Convert the amber coordinate/topology files back to PDB format using ambpdb
        ambpdb_command = f"ambpdb -p {receptor_prmtop} -c {receptor_inpcrd} > {tleap_out_pdb}"
        try:
            subprocess.run(ambpdb_command, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except Exception as e:
            print(f"❌ ambpdb failed: {e}")
            return None

        # Rename WAT back to HOH
        with open(tleap_out_pdb, 'r') as f:
            content = f.read()
        content = content.replace(' WAT ', ' HOH ')
        with open(tleap_out_pdb, 'w') as f:
            f.write(content)

        # Clean up tleap input file
        os.remove(tleap_in_file)

        # Remove TER cards from the ambpdb output PDB file
        with open(tleap_out_pdb, 'r') as f:
            lines = f.readlines()
        ...
        print(f"✅ ambpdb processing complete: {tleap_out_pdb}")

        return tleap_out_pdb

    def _create_residues_key_list(self, pdb_file):
        
        from moldf import read_pdb

        # Read the original PDB file using moldf

        moldf_dict = read_pdb(pdb_file)
        moldf_df = moldf_dict['_atom_site']

        # Extract residue information from the original PDB
        file_residues = []
        for idx, row in moldf_df.iterrows():
            # Extract fields of interest
            record_name = row['record_name'].strip()
            res_name = row['residue_name'].strip()
            res_number = row['residue_number']
            res_chain = row['chain_id'].strip()
            
            if record_name == 'TER':
                continue # Skip line because it is a TER card
            
            if res_chain == '': # Will build residue key for tleap format (or non specified pdb diles) that does not includes chain info
                res_key = res_name + str(res_number)
            
            elif res_chain != '': # Will build residue key including chain info
                res_key = res_name + str(res_number) + '_' + res_chain
                
            if res_key not in file_residues:
